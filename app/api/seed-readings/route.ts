...
export async function GET() {
  try {
    ...
    // Check if we already seeded to avoid infinite documents
    const count = await Reading.countDocuments();
    if (count > 0) {
      // Clear out existing if we want a fresh 365
    ...
    return NextResponse.json({ success: true, message: `Successfully seeded ${readingsToInsert.length} daily readings for 2026 to Atlas database.` });
