...
interface Item { _id: string; name: string; feastDay?: string; era?: string; bornYear?: number; deathYear?: number; description?: string; image?: string; }

const empty = { name: "", feastDay: "", description: "", image: "", bornYear: "", deathYear: "", nationality: "" };
...
export default function AdminSaints() {
  ...
  const save = async () => {
    setSaving(true);
    await fetch("/api/admin/saints", { method: editing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(editing ? { ...form, _id: editing } : form) });
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", flexWrap: "wrap", gap: "1rem" }}>
        <div><h1 style={{ fontFamily: "Georgia, serif", color: "#1a2744", fontSize: "1.5rem", margin: "0 0 0.2rem" }}>Saints</h1><p style={{ color: "#888", fontSize: "0.8rem", margin: 0 }}>{total} saints</p></div>
        <div style={{ display: "flex", gap: "0.5rem" }}>
          <input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === "Enter" && load(1)} placeholder="Search saints..." style={{ ...inputStyle, width: "200px" }} />
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
          <div style={{ background: "#fff", borderRadius: "12px", padding: "2rem", width: "100%", maxWidth: "600px", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
              {[["name","Name","text"],["feastDay","Feast Day (e.g. Jan 1)","text"],["bornYear","Born Year","number"],["deathYear","Death Year","number"],["nationality","Nationality","text"],["patronOf","Patron Of","text"]].map(([k,l,t]) => (
                <div key={k}><label style={labelStyle}>{l}</label><input type={t} style={inputStyle} value={(form[k] as string) || ""} onChange={e => setForm(f => ({ ...f, [k]: e.target.value }))} /></div>
              ))}
            </div>
            <div style={{ marginTop: "1rem" }}><label style={labelStyle}>Image URL</label><input style={inputStyle} value={(form.image as string) || ""} onChange={e => setForm(f => ({ ...f, image: e.target.value }))} /></div>
            <div style={{ marginTop: "1rem" }}><label style={labelStyle}>Description</label><textarea style={{ ...inputStyle, height: "120px", resize: "vertical" }} value={(form.description as string) || ""} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} /></div>
            <div style={{ display: "flex", gap: "0.75rem", justifyContent: "flex-end", marginTop: "1.5rem" }}>
              ...
              <button onClick={save} disabled={saving} style={{ padding: "0.6rem 1.25rem", background: "#1a2744", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: 600 }}>{saving ? "Saving..." : "Save Saint"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ background: "#fff", borderRadius: "10px", border: "1px solid rgba(0,0,0,0.07)", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.82rem" }}>
          <thead><tr style={{ background: "#f8f8f6", borderBottom: "1px solid #eee" }}>
            {["Name", "Feast Day", "Era", "Actions"].map(h => <th key={h} style={{ padding: "0.75rem 1rem", textAlign: "left", fontWeight: 600, color: "#555", fontSize: "0.75rem", textTransform: "uppercase" }}>{h}</th>)}
          </tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={4} style={{ padding: "3rem", textAlign: "center", color: "#888" }}>Loading...</td></tr>
              : items.length === 0 ? <tr><td colSpan={4} style={{ padding: "3rem", textAlign: "center", color: "#888" }}>No saints found.</td></tr>
              : items.map((item, i) => (
                <tr key={item._id} style={{ borderBottom: "1px solid #f0f0f0", background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                  ...
                  <td style={{ padding: "0.75rem 1rem", color: "#555" }}>{item.feastDay || "—"}</td>
                  <td style={{ padding: "0.75rem 1rem", color: "#555" }}>{item.bornYear && item.deathYear ? `${item.bornYear}–${item.deathYear}` : item.era || "—"}</td>
